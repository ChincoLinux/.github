...
1. **Cambios de código**: PR revisado y aprobado por `core-devs` antes de merge (nunca push directo a `main`).
2. **Cambios de doctrina/política**: propuesta como issue + PR en este repositorio (`.github`); decisión por consenso de `core-devs`.
...
Los cambios a este documento se hacen por PR en este repositorio, revisado por `core-devs`.